...
vel_S = vel_ratio*[vel_sx 0 0];
vel_P = vel_ratio*[-vel_px 0 0];
vel_t = {vel_S vel_P};
...
p = 1;
     for separation2 = 0.8:0.1:7
        ...
        for phi = angle_list
            ...
            a = dist(CM_pos,pos_p);
            j = sqrt(G*CM_mass*a*(1-e_a^2));
            A = pi*a^2*sqrt(1-e_a^2);
            pperiod = abs(2*A/j);
        ...
        %--------------------------------------------------------------------------
            m=1; eps=0; t0 =0; tf=3*500;
            tot_num = length(pos_t); n = tot_num;

            y0 = [pos_t{3} vel_t{3}];length(y0);
...
faster_vel = [faster_vel vel_s];
vel_UCO_cell{vv} = UCO_val;
vel_LCO_cell{vv} = LCO_val;
vv = vv + 1;
end
